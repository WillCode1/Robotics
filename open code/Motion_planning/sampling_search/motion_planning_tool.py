...
def check_point(point, map_img):
    ...
    point = np.mat(point)
    feasibility = True
    if map_img.shape[0] > int(point[:, 0]) >= 0 and map_img.shape[1] > int(point[:, 1]) >= 0:
...
def check_path(point_one, point_two, map_img):
    ## 路径检查工具：采样点数，取路径横向与纵向长度的较大值，保证每个像素都能验证到
    step_length = max(abs(point_one[0, 0] - point_two[0, 0]), abs(point_one[0, 1] - point_two[0, 1]))
    path_x = np.linspace(point_one[0, 0], point_two[0, 0], step_length + 1)
    path_y = np.linspace(point_one[0, 1], point_two[0, 1], step_length + 1)
...
def A_star_list(vertex, self_index, goal_index, parent_index, historic_cost):
    ## A*算法的路径代价表计算工具
    historic_cost = historic_cost
    heuristic_cost = straight_distance(vertex[self_index, :], vertex[goal_index, :])
...
def tree_plot(map_img, rrt_tree):
    ...
    while point_a_index != 0:
        ...
        cv2.line(map_img, vertex_tuple[int(point_a_index)][0: 2],
                 vertex_tuple[int(point_b_index)][0: 2], (0, 0, 255), 4)
        point_a_index = point_b_index
    cv2.imshow("地图", map_img)  # 转为RGB显示
    cv2.waitKey()
