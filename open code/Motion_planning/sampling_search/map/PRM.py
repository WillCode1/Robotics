...
def check_path(point_current, point_other, map):
    ...
    step_length = max(abs(point_current[0] - point_other[0]), abs(point_current[1] - point_other[1]))
...
def PRM():
    ...
    for i in range(sample_num + 2):
        for j in range(sample_num + 2):
            # 如果距离小于50且路径不碰撞
            if (straight_distance(vertex[i, :], vertex[j, :]) <= 1000.0) and (
                    check_path(vertex[i, :], vertex[j, :], img_binary)):
